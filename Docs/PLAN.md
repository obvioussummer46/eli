...
- [x] Alternate app icons — SHIPPED 01.09.2026 as a free choice under
      „Mehr" → App-Symbol: „Eli", an *own* serif-wordmark design in the
      school site's red (`#ce4b51` → dark), deliberately not the school's
      logo (that would need the school's okay). Generated by
      a CoreGraphics script; may move into the supporter bundle once IAP
      exists — revisit the free/paid split then.
- [x] Family Sharing ON for every non-consumable and the subscription.
- Prerequisites: ~~paid developer account~~ — in place since
  09.09.2026; App Store Connect banking/tax setup (Paid Applications
  agreement) still to do, see `Docs/APP-STORE.md` „Before submitting“.
- Revised 04.09.2026 (`Docs/MONETIZATION.md` is the current word): the
  three utility widgets, notifications and the digest stay free; three
  *additional* widgets, icon packs and a few extras are Pro, as a
  one-time purchase or a yearly plan. The original objection — never
  paywall the child's timetable — still holds and is why the split is
  drawn where it is.
...
### 3.3 Live Activity — v1 SHIPPED 01.09.2026

Current/next lesson with a system-driven countdown on the lock screen
and in the Dynamic Island. Off by default („Mehr" → Live-Aktivität);
iOS additionally lets the user forbid Live Activities per app, and the
lock-screen card can always be swiped away.

The honest constraint, documented in `LessonActivityController`: there
is no push channel, so the activity is only as fresh as the app's last
run (foreground activation or the ~4h background refresh). Between
runs only the countdown timers tick; once the shown lesson's end passes
(`staleDate`) the card says „Stunde vorbei" instead of pretending. If
the family finds the staleness annoying rather than charming, the fix
is push-based updates — which needs a server and is therefore out of
character for this app; more likely the feature just gets turned off.
**Needs a real device to verify** — Live Activities barely work in the
simulator.
...
2. ~~Is the paid developer account happening?~~ Answered 01.09.2026:
   not yet, planned soon. **Done 09.09.2026** — the account exists, and
   nothing in the code waits on it: the store, the Pro gates and the
   Siri shortcut were built against the free team and only need the
   paid team's id in the project. What remains is App Store Connect
   work, listed in `Docs/APP-STORE.md` „Before submitting“.
